...
| 製品ライセンス | ソースコードはMIT（Copyright (c) 2026 af）、素材はCC BY 4.0、ブランド素材とKSO闘龍で描いた画像は許諾しない（2026-09-25決定。`LICENSE`・`LICENSE-ASSETS.md`） |
| 第三者ライセンス表記 | 公開時に決定。駒テクスチャには昭和書体「闘龍」（KSO闘龍）で描画した文字が含まれる（フォントファイルは同梱しない。使用許諾 https://designpocket.jp/font/detail/23984 ）。フォントの使用許諾と配布物は2026-09-24に監査済み（判定A、Tests/ACCEPTANCE-1.3.0.md）。KSO闘龍で描いた画像は再許諾しない（`LICENSE-ASSETS.md`） |

## ライセンス（本プロジェクト固有、2026-09-25〜）

- 規約資料の既定のライセンス方針を最初に適用したプロジェクト。ソースコードは `LICENSE`（MIT、Copyright (c) 2026 af）、コード以外の素材と例外は `LICENSE-ASSETS.md`。
- 作者が作成した素材はCC BY 4.0。ブランド素材（製品名・ブログ名・AF-AI LAB、ロゴ・タイトル画像、アプリアイコン、マスコット）と、KSO闘龍で描いた画像（`piece_*.png`（back/side除く）、`label_hq.png`、`Generated/Previews/piece_contact_sheet.png`）は許諾しない。
- 新しい素材やファイルを追加したら、`LICENSE-ASSETS.md` のどの区分に入るかを確認し、必要なら追記する。KSO闘龍で描いた画像やブランド素材を追加した場合は除外の一覧に加える。
- 第三者の素材・ライブラリを追加する場合は元のライセンスに従い、同梱物と表記をこのファイルとREADMEに記録する。
- 同梱README（`Distribution/README-ja.txt`）にライセンスの要点を書く。既存の公開ZIPは上書きしない。
...
## 検証記録と完了報告（本プロジェクト固有、2026-09-24〜）

経緯：1.2.0の効果音は、再生要求が視覚イベントと同じタイミングで出ることだけを検証し、AudioListenerがなく実際には無音だった（1.2.1で修正）。`Tests/ACCEPTANCE-1.2.0.md` の効果音の記述は出力まで確認したように読める。1.2.1の報告で「リリース候補の段階で訂正する」としたが、訂正されないまま残った。ユーザーの判断により同ファイルは訂正せず当時の記録として残し、正しい経緯はHISTORY 1.2.1と本節に置く。再発防止として次を守る。

- 検証記録・完了報告では、何を確かめたかを段階で書き分ける。(a) 内部の要求・呼び出し・イベントが発生したこと、(b) 実際の出力（音声ミックスの出力値、画面の描画、OS経由の入力など）、(c) 人の知覚による確認（聴感・見た目・操作感）。確かめていない段階を確認済みのように書かない。実施していない段階は「未確認」と明記する。
- 利用者が知覚する出力を持つ機能（音・表示・入力）は、少なくとも(b)の検証を1つ持つ。例：`-audioprobe`（出力ミックスのピーク）、`-modalprobe`（OS経由の実クリック）。(a)だけの検証で「動作確認済み」としない。
- 報告で「後で訂正・対応する」とした事項は、その場で追跡できる場所（HISTORYの該当版の未完了事項、または受入記録の「持ち越し」欄）に書き、次のリリース候補の検証で消し込む。消し込めない場合はユーザーに報告する。チャット上の約束だけにしない。
- 過去の検証記録は、ユーザーの指示がない限り書き換えない。誤りや不足は、次の版の受入記録やHISTORYなど新しい記録で補う。
